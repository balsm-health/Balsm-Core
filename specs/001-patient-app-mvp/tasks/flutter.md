...
- [X] T035ag [P] [Flutter] Create `BalsmMedRow` widget at `../balsm_app/packages/core/lib/src/kit/widgets/balsm_med_row.dart` — ports prototype `.med-row` (40pt icon with tone-coded background + name + dose + status pill OR action button). Tone enum: `info` (hue-blue), `controlled` (hue-violet per FR-020), `success` (mint). Used on `meds-list` and `home` today summary
