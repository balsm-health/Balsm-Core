...
## Sync history
- 2026-09-23 — **circular-safe vertical lockup.** New generated avatars: `brand/logo-vertical-social.svg` (transparent) and `-white.svg` (white plate), plus `.png` at 1024 and 400. Unlike `icon-social*` these carry the **full lockup** — mark *and* wordmark — sized so nothing is lost when a platform masks the square to a circle. The size comes from the content's minimal enclosing circle (`_vertical_social_layout()`), not the ink box's diagonal: the mark contributes its known ring circle and the wordmark its four corners, which buys ~11% more size than fitting the box would. Content fills 90% of the inscribed radius (`V_SOCIAL_FILL`); measured furthest ink is exactly 0.900 R, zero pixels outside. Use for avatars where the icon alone is not enough to identify the account.
